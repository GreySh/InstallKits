...
def _build_dispatch_details(components, quantity, old_stocks=None):
    ...
    for comp in components:
        ...
        if disc_id and disc_qty > 0:
            ...
            old_q = old_stocks.get(('disc', disc_id), 0)
            ...
            details['components'].append({
                ...
                'old_quantity': old_q,
                'new_quantity': old_q - total_disc_needed,
            })

        if box_id and box_qty > 0:
            total_box_needed = box_qty * quantity
            old_q = old_stocks.get(('box', box_id), 0)
            ...
            details['components'].append({
                ...
                'old_quantity': old_q,
                'new_quantity': old_q - total_box_needed,
            })
...
def dispatch_product(product_id, quantity, dispatch_date=None):
    ...
    for comp in components:
        ...
        if disc_id and disc_qty > 0:
            old_stocks[('disc', disc_id)] = get_stock_disc_quantity(disc_id)
            adjust_stock_disc(disc_id, disc_qty * quantity, 'subtract', log_operation=False)
        if box_id and box_qty > 0:
            old_stocks[('box', box_id)] = get_stock_box_quantity(box_id)
            adjust_stock_box(box_id, box_qty * quantity, 'subtract', log_operation=False)

    details = _build_dispatch_details(components, quantity, old_stocks)
...
def adjust_stock_disc(disc_id, quantity, operation='set', log_operation=True):
    ...
    # Если количество не изменилось — не логируем операцию
    ...
    if log_operation:
        ...
        details = {
            'components': [{'type': 'disc', 'name': disc_name, 'quantity': new_quantity,
                            'old_quantity': current_quantity, 'new_quantity': new_quantity}],
    ...
    if result:
        details = {
            'components': [{'type': 'disc', 'name': disc_name, 'quantity': quantity,
                            'old_quantity': current, 'new_quantity': current + quantity}],
...
def fixate_stock():
    """Зафиксировать текущие остатки на сегодняшнюю дату.
    Если фиксация за сегодня уже есть — обновить её, иначе создать новую."""
    from datetime import datetime

    db = get_database()
    fixations_table = get_table(db, 'stock_fixations')
    ...
    fixation = {
        ...
        'discs': [
            {'disc_id': d['disc_id'], 'name': d['disc_name'],
             'quantity': d['quantity'], 'description': d.get('description', '')}
            for d in discs
        ],
        'boxes': [
            {'box_id': b['box_id'], 'name': b['box_name'],
             'quantity': b['quantity'], 'description': b.get('description', '')}
            for b in boxes
        ],
    }

    Query_ = Query()
    existing = fixations_table.get(Query_.date == today)
    if existing:
        fixations_table.update(fixation, doc_ids=[existing.doc_id])
        return existing.doc_id
    else:
        return fixations_table.insert(fixation)


def get_all_fixations():
    """Получить все фиксации остатков (новые первые)."""
    db = get_database()
    fixations_table = get_table(db, 'stock_fixations')
    return sorted(fixations_table.all(), key=lambda f: f.get('date', ''), reverse=True)


def get_fixation_by_id(fixation_id):
    """Получить фиксацию по ID."""
    db = get_database()
    fixations_table = get_table(db, 'stock_fixations')
    return fixations_table.get(doc_id=fixation_id)
