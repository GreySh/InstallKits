...
from data import get_operations, get_product_by_id, get_all_products, get_all_discs, get_all_boxes
...
class OperationsTab(ctk.CTkFrame):
    def __init__(self, master):
        ...
        ctk.CTkLabel(row2, text="Компонент:").pack(side="left", padx=(10, 2))
        self.component_var = ctk.StringVar(value="Все")
        component_names = ["Все"] + sorted(set(
            [d['name'] for d in get_all_discs()] + [b['name'] for b in get_all_boxes()]
        ))
        self.component_combo = ctk.CTkComboBox(row2, values=component_names, variable=self.component_var,
                                                width=150, command=lambda _: self.load_operations())
        self.component_combo.pack(side="left", padx=2)
        ...
        self.ops_tree.column("date", width=120, anchor="w", stretch=False)
        self.ops_tree.column("type", width=140, anchor="w", stretch=False)
        self.ops_tree.column("product", width=110, anchor="w", stretch=False)
        self.ops_tree.column("quantity", width=75, anchor="center", stretch=False)
        self.ops_tree.column("details", width=250, anchor="w", stretch=True)
    ...
    def load_operations(self):
        ...
        filter_component = self.component_var.get()
        ...
        # Фильтр по компоненту (по названию)
        if filter_component != "Все":
            filtered = []
            for op in operations:
                details = op.get('details', {})
                if isinstance(details, dict):
                    for c in details.get('components', []):
                        if c.get('name') == filter_component:
                            filtered.append(op)
                            break
            operations = filtered
        ...
        for op in operations:
            ...
            if product_id:
                ...
                details_tmp = op.get('details', {})
                if isinstance(details_tmp, dict):
                    comps = details_tmp.get('components', [])
                    if comps:
                        product_name = ', '.join(c.get('name', '') for c in comps)
            ...
            if isinstance(details, dict):
                ...
                if components:
                    parts = []
                    for c in components:
                        old_q = c.get('old_quantity')
                        new_q = c.get('new_quantity')
                        name = c.get('name', '')
                        ctype = c.get('type', '').upper()
                        if old_q is not None and new_q is not None:
                            parts.append(f"{ctype}: {name} ({old_q} → {new_q})")
                        else:
                            qty = c.get('quantity', c.get('total_quantity', ''))
                            parts.append(f"{ctype}: {name} ({qty} шт.)")
    ...
    def sort_by(self, column):
        ...
        if column in numeric_cols:
            items.sort(key=lambda x: int(x[3]) if x[3] else 0, reverse=self.sort_reverse)
        elif column in date_cols:
            items.sort(key=lambda x: x[0] if x[0] else "", reverse=self.sort_reverse)
        else:
            col_index = {"type": 0, "product": 1, "details": 3}.get(column, 0)
            items.sort(key=lambda x: x[col_index] if x[col_index] else "", reverse=self.sort_reverse)

        for item_id in self.ops_tree.get_children():
            self.ops_tree.delete(item_id)

        for values in items:
            self.ops_tree.insert("", "end", values=values)
